...
$HUDIntuneInstall = "$Env:Programfiles\HUD Tools\EndpointManager"
Start-Transcript -Path "$HUDIntuneInstall\Log\$PackageName-ScriptRun.log" -Force
try{
    ...
    if($task_existing.Description -like "Version $Version*"){
        ...
        # script path
        $script_path = "$HUDIntuneInstall\Data\$PackageName.ps1"
