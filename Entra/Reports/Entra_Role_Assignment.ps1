...
foreach ($role in $roles) { Add-Member -InputObject $role -MemberType NoteProperty -Name roleDefinition1 -Value ($roles1 | Where-Object {$_.id -eq $role.id}).roleDefinition } #and another fix needed as PowerShell populates empty roleDefinition property...
...
if ($IncludePIMEligibleAssignments) {
    ...
    $roles += (Get-MgRoleManagementDirectoryRoleEligibilitySchedule -All -ExpandProperty roleDefinition,principal -Verbose:$false -ErrorAction Stop | Select-Object id,principalId,directoryScopeId,roleDefinitionId,status,principal,@{n="roleDefinition1";e={$_.roleDefinition}})
    ...
    foreach ($roleactivation in $roleactivations) {
        ...
        $roles | Where-Object {($_.id -eq $roleactivation.RoleAssignmentOriginId) -and ($_.Id -eq $roleactivation.Id)} | ForEach-Object { Add-Member -InputObject $_ -MemberType NoteProperty -Name "Duplicate" -Value $true }
    }
}

if (!$roles) { Write-Verbose "No valid role assignments found, verify the required permissions have been granted?"; return }

$rtemp = $roles | Where-Object {!$_.Duplicate}
Write-Verbose "A total of $($rtemp.count) role assignments were found"
Write-Verbose "$(($rtemp | Where-Object {$_.directoryScopeId -eq "/"}).Count) are tenant-wide and $(($rtemp | Where-Object {$_.directoryScopeId -ne "/"}).Count) are AU-scoped."
Write-Verbose "$(($rtemp | Where-Object {!$_.status}).Count) roles are permanently assigned, you might want to address that!"
#endregion Roles

#region PAG
if ($IncludePAGAssignments) {
    #Get the set of roles with Group principal
    $Proles = $roles | Where-Object {$_.Principal.AdditionalProperties.'@odata.type' -eq '#microsoft.graph.group'} #not necessarily PIM-managed group (can be "old" PAG, in both cases the role-assignable flag should be set)
...
foreach ($role in $roles) {
    ...
    if (!$role.status) { #if the role is permanently assigned, we don't need to check the role activations
        ...
        if ($role.principal.AdditionalProperties.'@odata.type' -eq '#microsoft.graph.group') {
            $activeRole = $roleactivations | Where-Object {($_.roleDefinitionId -eq $role.roleDefinitionId) -and ($role."Active group members".ContainsKey($_.principalId)) -and ($_.MemberType -eq "Group")}
            $role | Add-Member -MemberType NoteProperty -Name "Activated for" -Value (($activeRole | ForEach-Object { $($role."Active group members"[$_.principalId]) }) -join ";")
        }
        else {
            $activeRole = $roleactivations | Where-Object {($_.roleDefinitionId -eq $role.roleDefinitionId) -and ($_.PrincipalId -eq $role.PrincipalId)}
            $role | Add-Member -MemberType NoteProperty -Name "Activated for" -Value $null
        }

        $role | Add-Member -MemberType NoteProperty -Name "Start time" -Value (&{if ($activeRole.startDateTime) {(Get-Date($activeRole.startDateTime | Select-Object -Unique | Sort-Object | Select-Object -First 1) -format g)} else {$null}})
        $role | Add-Member -MemberType NoteProperty -Name "End time" -Value (&{if ($activeRole.endDateTime) {(Get-Date($activeRole.endDateTime | Select-Object -Unique | Sort-Object -Descending | Select-Object -First 1) -format g)} else {$null}})
...
#format and export
$File = "C:\HUD\06_Reporting\$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_EntraRoleInventory.xlsx"
Write-Verbose "Output exported to $($File)"

$report | Export-Excel -Path $($File) -WorksheetName Inventory -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow
