...
if ($Mode -eq "Install") {
    try {
		...
		$SetupFilePath = (Join-Path -Path $SetupFolder -ChildPath "safeq-cloud-client-3.41.0-release-setup.exe").ToString()
		...
		try {
			...
			[string]$Arguments = "/S /GATEWAYADDRESS=aucklandcentralhub.au.ysoft.cloud /ACCOUNTDOMAIN=aucklandcentralhub.au.ysoft.cloud /AUTHTYPE=5 /REMEMBERLOGIN=true /PAPER=A4"
