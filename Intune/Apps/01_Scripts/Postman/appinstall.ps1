...
$AppName = "Postman Desktop App v10.24"
...
if ($Mode -eq "Install") {
    try {
		...
		$SetupFilePath = (Join-Path -Path $SetupFolder -ChildPath "Postman-win64-Setup.exe").ToString()
		...
		try {
			...
			Write-LogEntry -Value "Starting $Mode of Postman" -Severity 1
			[string]$Arguments = "-s"
