<#
.SYNOPSIS
    7-Zip File Archive Manager.

.DESCRIPTION
    Script to install 7-Zip File Archive Manager.
...
if ($Mode -eq "Install") {
    try {
		...
		$SetupFilePath = (Join-Path -Path $SetupFolder -ChildPath "7z2301-x64.exe").ToString()
		...
		try {
			...
			Write-LogEntry -Value "Starting $Mode of 7-Zip File Archive Manager" -Severity 1
			[string]$Arguments = "/S"
	...
	try {
		$MyApp = Get-InstalledApps -App "7-zip*"
