...
if($Remove){
    ...
    $UsersReg | %{Remove-Item -Path $_ -Recurse -Force}
    ...
    if(Is-Admin){
        ...
        $CurrentUsers = Get-ProcessUser |?{$_.Username -eq $env:USERNAME}
    }
    ...
    foreach($U in $CurrentUsers){
        ...
        switch ($OfficeVersion){
            ...
            default    {
                if($OfficeVersion -match "Office(.*)"){
                    ...
                    $OficeReg = "HKU:\$($U.SID)\SOFTWARE\Microsoft\Office\16.0"
                }
            }
        }
        ...
        if($WordOptions.officestartdefaulttab -ne 1){New-ItemProperty -Name officestartdefaulttab -Value 1 -PropertyType dword -Path $WordReg -Force | Out-Null}
