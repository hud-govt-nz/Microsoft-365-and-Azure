<#
.SYNOPSIS
    Citrix Receiver App Install Script

.DESCRIPTION
    Script to install Citrix Receiver App
...
$AppName = "Citrix Receiver 4.12"
$AppVersion = "4.12"
$Installer = "CitrixReceiver.exe" # assumes the .exe or .msi installer is in the Files folder of the app package.
$InstallArguments = "/silent" # Optional
...
switch ($Mode) {
    ...
    "Uninstall" {
        try {
            ...
            # Uninstall App
            $exePath = "C:\ProgramData\Citrix\Citrix Receiver 4.12\TrolleyExpress.exe"
            $uninstallArgs = "/silent /uninstall /cleanup"
            $uninstallProcess = Start-Process -FilePath $exePath -ArgumentList $uninstallArgs -PassThru -Wait -ErrorAction Stop
