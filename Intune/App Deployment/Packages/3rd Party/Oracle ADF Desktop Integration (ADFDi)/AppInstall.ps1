<#
.SYNOPSIS
    Oracle ADF Desktop Integration Excel Add-in

.DESCRIPTION
    Script to install Oracle ADF Desktop Integration Excel Add-in.
...
$DotNetRelease = Get-DotNetVersion

# Check if .NET Framework 4.8.1 is installed
if ($DotNetRelease -eq 533320) {
    Write-Host ".NET Framework 4.8.1 is installed." -ForegroundColor Green
} else {
    Write-Host ".NET Framework 4.8.1 is not installed." -ForegroundColor Red
    exit 1
}

# Application Variables
$AppName = "Oracle ADF Desktop Integration Add-In for Excel"
$AppVersion = "5.2.0.26990"
$Installer = "adfdi-excel-addin-installer-all-users.msi" # assumes the .exe or .msi installer is in the Files folder of the app package.
$InstallArguments = "/quiet" # Optional
