...
.DESCRIPTION
    Script to install Cisco AnyConnect VPN
...
$InstallArguments = "TRANSFORMS=CSCTransform.mst /qn /norestart ALLUSERS=1" # Optional
...
switch ($Mode) {
    ...
    "Uninstall" {
        try {
            ...
            # Uninstall App
            $uninstallProcess = Start-Process "C:\Program Files (x86)\Cisco\Cisco Secure Client\Uninstall.exe" -ArgumentList "-remove" -PassThru -Wait -ErrorAction Stop
