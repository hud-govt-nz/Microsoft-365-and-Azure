...
$AppName = "Microsoft Windows Desktop Runtime - 8.0.11"
$AppVersion = "8.0.11.34221"
$Installer = "windowsdesktop-runtime-8.0.11-win-x64.exe" # assumes the .exe or .msi installer is in the Files folder of the app package.
    ...
    "Install" {
        try {
            ...
            try {
                ...
                # Stop current printix processes
                $processes = Get-Process | Where-Object { $_.Name -like '*printix*' }   
                foreach ($process in $processes) {  
                    Stop-Process -Id $process.Id -Force  
                }  
                Write-LogEntry -Value "Printix processes have been stopped" -Severity 1
