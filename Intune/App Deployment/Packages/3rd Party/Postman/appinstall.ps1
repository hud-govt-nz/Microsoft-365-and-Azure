...
$AppName = "Postman"
$AppVersion = "11.1.14"
$Installer = "Postman-win64-Setup.exe" # assumes the .exe or .msi installer is in the Files folder of the app package.
$InstallArguments = "-s" # Optional
$UninstallArguments = "--uninstall -s" # Optional
...
switch ($Mode) {
    ...
    "Uninstall" {
        try {
            # install is in USER Context, so we need to get the SID of the current user
            $SID = Get-CurrentUserSID
            
            # Get Uninstall Path
            $UninstallPath = [string](Get-ItemProperty Registry::HKEY_USERS\$SID\Software\Microsoft\Windows\CurrentVersion\Uninstall\Postman).uninstallstring.Replace(' --uninstall', '')
            ...
            if ($uninstallProcess.ExitCode -eq "0") {
                ...
                Write-LogEntry -Value "Leftover files deleted." -Severity 1
