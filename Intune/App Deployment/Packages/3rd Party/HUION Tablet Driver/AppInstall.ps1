<#
.SYNOPSIS
    HuionTablet_WinDriver

.DESCRIPTION
    Script to install HuionTablet_WinDriver
...
$AppName = "HuionTablet_WinDriver"
$AppVersion = "15.7.6.1314"
$Installer = "HuionTablet_WinDriver_v15.7.6.1314.exe" # assumes the .exe or .msi installer is in the Files folder of the app package.
...
switch ($Mode) {
    ...
    "Uninstall" {
        try {
            ...
            # Split the uninstall string into executable path and arguments
            $splitIndex = $AppToUninstall.UninstallString.IndexOf(".exe") + 4 # +4 to include ".exe" in the path
            $ExecutablePath = $AppToUninstall.UninstallString.Substring(0, $splitIndex)
            $UninstallArguments = $AppToUninstall.UninstallString.Substring($splitIndex).Trim() + " /qb"
            
            # Uninstall App
            $uninstallProcess = Start-Process $ExecutablePath -ArgumentList $UninstallArguments -PassThru -Wait -ErrorAction stop 
