<#
.SYNOPSIS
    Te Reo Maori Addin (Outlook)

.DESCRIPTION
    Script to install Te Reo Maori Addin (Outlook)
...
$AppName = "Te Reo Maori Addin"
$AppVersion = "1.0"
$Installer = "setup.msi" # assumes the .exe or .msi installer is in the Files folder of the app package.
$InstallArguments = "ALLUSERS=1 /qn" # Optional
...
switch ($Mode) {
    "Install" {
        try {
            ...
            Copy-Item -Path "$SetupFolder\TeReoGreetings.csv" -Destination "C:\ProgramData\TeReoAddin\"
    ...
    "Uninstall" {
        try {
            ...
            $Result = (($AppToUninstall.UninstallString -split ' ')[1] -replace '/I','/X ') + ' /qb'
            ...
            if ($uninstallProcess.ExitCode -eq "0") {
                ...
                try {
                    ...
                    # Delete Greetings CSV
                    Remove-Item -Path "C:\ProgramData\TeReoAddin\TeReoGreetings.csv" -Force -Confirm:$false
                    Write-LogEntry -Value "TeReoGreetings.csv has been removed from C:\ProgramData\TeReoAddin" -Severity 1
