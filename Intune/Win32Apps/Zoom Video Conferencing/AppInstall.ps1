...
$AppName=[string]'Zoom(64bit)'
$AppVersion="5.14.8"
$AppValidationFile="$validation\$AppName.txt"
$AppLog="$logs\$AppName.log"
$AppInstallFile= "ZoomInstallerFull.exe"
$AppInstallArguments="/quiet /qn /norestart ZoomAutoUpdate=1"
    ...
    'uninstall' {
        if ($Result) {
            try {
                ...
                $outfile = "$Path\UninstallZoom.zip"
                $wc = New-Object net.webclient
                $wc.Downloadfile($url, $outfile)

                # Extract Uninstaller zip file
                $extractPath = "$Path\"
                Expand-Archive -Path $outfile -DestinationPath $extractPath

                # Run Zoom uninstaller
                $uninstallProcess = Start-Process $Path\CleanZoom.exe -ArgumentList '/silent' -PassThru -Wait -ErrorAction Stop
