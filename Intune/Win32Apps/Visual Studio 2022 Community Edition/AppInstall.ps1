<#
.APP: Microsoft Visual Studio 2022 Community Edition
.AUTHOR: Ashley Forde
.DATE: 21 June 2023
#>
...
$AppName=[string]'Microsoft Visual Studio 2022 Community Edition'
$AppVersion="17.6.4"
    ...
    'install' {
        if ($Result[0]) {
            Write-Log -Path $AppLog -Value "[$(Get-Date)] $AppName is currently installed with version $AppVersion."
                ...
                Write-Log -Path $AppLog -Value "[$(Get-Date)] $AppName version $AppVersion was installed successfully"
                    } catch {
