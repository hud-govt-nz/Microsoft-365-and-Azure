<#
.APP: Microsoft Azure Storage Explorer
.AUTHOR: Ashley Forde
.DATE: 16 May 2023
#>
...
$AppName=[string]'Microsoft Azure Storage Explorer'
$AppVersion="1.29.1"
$AppValidationFile="$validation\$AppName.txt"
$AppLog="$logs\$AppName.log"
$AppInstallFile = "Windows-StorageExplorer.exe"
$AppInstallArguments='/VERYSILENT /NORESTART /NOCANCEL /CLOSEAPPLICATIONS /RESTARTAPPLICATIONS /ALLUSERS'
...
switch ($args[0]) {
    ...
    'uninstall' {
        if ($Result) {
            try {
                # Uninstall App
                $uninstall_command = "C:\Program Files (x86)\Microsoft Azure Storage Explorer\unins000.exe"
