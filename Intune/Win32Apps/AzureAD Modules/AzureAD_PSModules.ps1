<#
.SYNOPSIS
    Install/Uninstall of PS modules with name AzureAD and AzureADPreview .

.DESCRIPTION
    Script installs the PowerShell Modules for all modules that begin with name AzureAD and AzureADPreview

.PARAMETER Mode
    Sets the mode of operation. Supported modes are Install or Uninstall.

.EXAMPLE 
    powershell.exe -executionpolicy bypass -file .\AzureAD_PSModule.ps1 -Mode Install
    powershell.exe -executionpolicy bypass -file .\AzureAD_PSModule.ps1 -Mode Uninstall
...
# Install/Uninstall Graph Modules
if ($Mode -eq "Install") {
	...
	try {
		...
		try {
			...
			$modules = @("AzureAD","AzureADPreview")
			...
			Write-LogEntry -Value "Error installing Microsoft Graph Module, Errormessage: $($_.Exception.Message)" -Severity 3
			return
		}
	}
	catch [System.Exception]{
...
elseif ($Mode -eq "Uninstall") {
	try {
		# Get all available modules that match the pattern 'AzureAD*'
		$modules = Get-Module -ListAvailable -Name "AzureAD*"

		# Check if modules were found
		if ($modules.count -eq 0) {
			Write-LogEntry -Value "No modules matching 'AzureAD*' were found." -Severity 1
			return
		}
