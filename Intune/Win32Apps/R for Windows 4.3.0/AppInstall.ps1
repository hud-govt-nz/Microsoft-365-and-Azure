...
$AppName=[string]'R for Windows 4.3.0'
$AppVersion="4.3.0"
$AppValidationFile="$validation\$AppName.txt"
$AppLog="$logs\$AppName.log"
$AppInstallFile= "R-4.3.0-win.exe"
$AppInstallArguments='/VERYSILENT'
...
switch ($args[0]) {
    ...
    'uninstall' {
        if ($Result) {
            try {
                # Uninstall App
                $uninstall_command = "C:\Program Files\R\R-4.3.0\unins000.exe"
