...
$AppName=[string]"Microsoft Visual Studio Code"
$AppVersion="1.78.2"
$AppValidationFile="$validation\$AppName.txt"
$AppLog="$logs\$AppName.log"
$AppInstallFile= "VSCodeSetup-x64-1.78.2.exe"
$AppInstallArguments="/VERYSILENT /NORESTART /NOCANCEL /CLOSEAPPLICATIONS /RESTARTAPPLICATIONS /SUPPRESSMSGBOXES /mergetasks=!runcode"
    ...
    'install' {
        ...
        'uninstall' {
            if ($Result) {
                try {
                    # Uninstall App
                    $uninstall_command = "C:\Program Files\Microsoft VS Code\unins000.exe"
